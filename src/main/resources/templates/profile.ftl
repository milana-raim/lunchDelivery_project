...
<#if user.role == "USER">
    <form class="form-center-content" method="post" action="/profile">
        ...
        Школа:
        <br>
        <select id="clientSchoolId" name="clientSchoolId" class="form-select" aria-label="Default select example" style="background-color: #f0f0f0" required>
            ...
            <option value=${school.id}>${school.name} ${school.country} ${school.city} ${school.street} ${school.house}</option>
            </#list>
        </select>
        <br>
        Класс/курс:
        <br>
        <input type="text" name="grade" id = "grade" class="form-control" placeholder=<#if user.grade??>${user.grade}</#if> >
        <br>
        Буква класса/номер группы:
        <br>
        <input type="text" name="className" id = "className" class="form-control" placeholder=<#if user.className??>${user.className}</#if>>
        <br>
...
<form class="form-center-content" method="post" action="/profile">
    ...
    Страна:
    <br>
    <input type="text" name="country" id = "country" class="form-control" <#if user.country??><#else>required</#if> placeholder=<#if user.country??>${user.country}</#if>>
    <br>
    Город:
    <br>
    <input type="text" name="city" id = "city" class="form-control" <#if user.city??><#else>required</#if> placeholder=<#if user.city??>${user.city}</#if>>
    <br>
    Улица:
    <br>
    <input type="text" name="street" id = "street" class="form-control" <#if user.street??><#else>required</#if> placeholder=<#if user.street??>${user.street}</#if>>
    <br>
    Дом:
    <br>
    <input type="text" name="house" id = "house" class="form-control" <#if user.house??><#else>required</#if> placeholder=<#if user.house??>${user.house}</#if>>
    <br>
    Индекс:
    <br>
    <input type="text" name="postalCode" id = "postalCode" class="form-control" <#if user.postalCode??><#else>required</#if> placeholder=<#if user.postalCode??>${user.postalCode}</#if>>
    <br>
    <button class="btn btn-lg btn-primary btn-block" type="submit">Отправить изменения</button>
